...
const orderFormSchema: z.ZodType<z.infer<typeof baseOrderFormSchema>> =
	baseOrderFormSchema
		...
		.refine(
			...
			{
				message:
					"Při objednávce dortu jsou povinné údaje o velikosti a příchuti",
				path: ["size"],
			},
		)
		.refine(
					...
					<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
						...
						<div className="space-y-8">
							...
							<div className="bg-white/80 rounded-lg p-5 border border-gray-100 shadow-sm">
								...
								{/* Cake form section */}
								<div
									className={`bg-pink-50/50 rounded-lg p-4 mb-6 transition-opacity duration-300 ease-in-out ${orderCake ? "block opacity-100" : "hidden opacity-0"}`}
								>
								...
								<div
									className={`bg-blue-50/50 rounded-lg p-4 mb-6 transition-opacity duration-300 ease-in-out ${orderDessert ? "block opacity-100" : "hidden opacity-0"}`}
								>
							...
							<div
								className={`${orderCake ? "block" : "hidden"} transition-opacity duration-300 ease-in-out`}
							>
								...
								<div className="mb-6">
									...
									<div>
										...
										<p className="text-sm text-gray-500 mt-1">
											Můžete nahrát více fotografií pro lepší představu o vašem
											vysněném dortu (držte Ctrl nebo Cmd pro výběr více
											souborů)
										</p>
										<p className="text-sm text-gray-500 mt-1">
											Podporované formáty: JPG, PNG, GIF, WEBP. Maximální
											velikost: 1 MB na soubor.
										</p>
									</div>
								</div>
							</div>
							...
							<p
								className={`${!orderCake && orderDessert ? "block" : "hidden"} text-center text-gray-500 italic py-4 transition-opacity duration-300 ease-in-out`}
							>

import { db, orderPhotos, type NewOrderPhoto, type OrderPhoto } from './index';
...
export async function saveOrderPhoto(
  orderId: number,
  file: File
): Promise<{ success: true; photo: OrderPhoto } | { success: false; error: string }> {
  try {
    // Validate file
    ...
    const [newPhoto] = await db.insert(orderPhotos).values(photoData).returning();
...
export async function saveOrderPhotos(
  orderId: number,
  files: File[]
): Promise<{ success: true; photos: OrderPhoto[] } | { success: false; error: string }> {
  try {
